...
export default function TribeCard({ tribe }) {
  ...
  return (
    <div className="flex items-center gap-3 bg-white rounded-2xl shadow-card p-3">
      <button onClick={() => nav(`/tribe/${tribe.id}`)} className="flex items-center gap-3 flex-1 min-w-0 text-left" aria-label={`Open ${tribe.name}`}>
        {tribe.img ? (
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-muted text-[13px] mt-0.5">{count} {count === 1 ? "Member" : "Members"}{tribe.location ? ` · ${tribe.location}` : ""}</p>
        </div>
      </button>
