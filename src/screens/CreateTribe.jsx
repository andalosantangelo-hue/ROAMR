...
const ACTIVITIES = [["outdoor","Outdoor"],["water","Water"],["wheel","Wheels"],["nature","Nature"],["snow","Snow"]];
...
export default function CreateTribe() {
  ...
  const submit = async () => {
    ...
    try {
      await addTribe({ name: name.trim(), file, categories, location: location.trim(), description: description.trim() });
      nav("/app/tribes");
    } catch (e) {
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="flex-1 px-6 pt-2 overflow-y-auto no-scrollbar">
        <button onClick={() => fileRef.current?.click()} aria-label="Add tribe photo"
        ...
        <label className="block mt-6 text-sm font-semibold text-ink/80 mb-2">Activities (select all that apply)</label>
        <div className="flex flex-wrap gap-2">
          {ACTIVITIES.map(([id, label]) => (
            <button key={id} type="button" onClick={() => toggleCat(id)}
              className={`px-3.5 py-2 rounded-full text-sm font-semibold transition ${categories.includes(id) ? "bg-brand-green text-white" : "bg-brand-tint text-brand-navy"}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Weekend Wanderers"
        ...
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} maxLength={500}
          placeholder="What's this tribe about? Who should join?"
      ...
      <div className="px-6 pb-7 pt-3">
        <button disabled={!canCreate} onClick={submit}
          className={`w-full rounded-xl py-4 font-semibold transition ${canCreate ? "bg-brand-green hover:bg-brand-greenDark text-white" : "bg-black/15 text-white/90 cursor-not-allowed"}`}>
